...
- `src/components/HtmlPreview.tsx`、`src/components/ReactPreview.tsx`、`src/components/DemoCard.tsx`、`src/components/PromptDisplay.tsx`、`src/components/SearchBar.tsx`
- `src/lib/types.ts`、`src/lib/demos.ts`、`src/lib/search/buildIndex.ts`、`src/lib/search/client.ts`
...
- 影响文件/目录：`src/components/HtmlPreview.tsx`、`src/components/ReactPreview.tsx`
- 产出物：两个可复用预览组件。
- AC：示例代码可渲染；沙箱安全属性生效。
...
- 影响文件/目录：`src/components/DemoCard.tsx`、`src/components/PromptDisplay.tsx`
- 产出物：列表与详情展示组件。
...
- 目标：提供 `getAllDemos()`、`getDemoBySlug(slug, locale)`；`title/description` 支持 `{en, zh}` 或字符串并回退。
- 影响文件/目录：`src/lib/demos.ts`、`_demos/**/meta.json`、`_demos/**/(index.html|App.tsx)`
- 产出物：数据层函数可供页面消费。
- AC：读取所有 demo；不存在的 `slug` 返回 404；`locale` 缺失回退至 `en`。
...
#### Story：构建列表索引与筛选函数
**Task：实现筛选逻辑与类型**
- 目标：基于 `ai.model/ai.agent/tags/type/query` 过滤列表。
...
**Task：实现页面与静态参数**
- 目标：完成 `layout.tsx`、`page.tsx`、`demo/[slug]/page.tsx`，并实现 `generateStaticParams` 覆盖 `en/zh`。
...
- 影响文件/目录：`src/components/SearchBar.tsx`、`src/hooks/useSearch.ts`
- 产出物：基本交互框架。
- AC：输入→建议触发；统计正常。
- DoD：状态与事件稳定；快测通过。
- 依赖与风险：索引生成在后续批次。
...
**Task：集成 `NextIntlClientProvider` 与 `messages/{locale}.json`**
- 目标：`/en` 与 `/zh` 文案加载正确；根路径重定向到 `/en`。
- 影响文件/目录：`messages/en.json`、`messages/zh.json`、`src/app/[locale]/layout.tsx`、`src/app/layout.tsx`
- 产出物：多语言路由与文案生效。
...
#### Story：本地化字段回退
**Task：统一解析与回退**
- 目标：解析 `{en, zh}` 或字符串，输出当前语言文本（默认 `en`）。
...
- 目标：覆盖标题/描述/标签/代码片段，包含 `version` 与 `generatedAt`。
...
- 影响文件/目录：`src/lib/search/client.ts`、`src/hooks/useSearch.ts`、`src/components/SearchBar.tsx`
- 产出物：可交互搜索体验。
- AC：准确率 ≥ 95%（样本）；性能达标。
...
## 验收与 DoD 总则
- 验收总则：
  - `/en`、`/zh` 路由完整；HTML/React 预览正确；Prompt/AI 信息完整；搜索准确率与性能达标；静态导出可部署。
- DoD 总则：
